...
2. 기준선을 위/아래 둘로 둬서, 그 사fy 아무것도 하지 않는다.
...
> [!TIP] 
핵심 은 수치를 미리 맞히는 것이 아니다. **evictable cached prefix block에 한정된 feedback loop**로 hot cache 피해는 줄이고 low-reuse cache 낭비는 회수하는 것, 그것이 이 설계의 전부다.
>