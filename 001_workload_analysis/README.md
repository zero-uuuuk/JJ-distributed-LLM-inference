...
Chat과 Agent workload를 동일한 request schema로 정규화한 뒤, vLLM에 Poisson arrival로 전송해 prompt 길이·KV cache reuse·latency를 측정한다.
...
- [`scripts/dataset.py`](scripts/dataset.py)
  - `load_dataset()`: ShareGPT·Terminal-Bench 원본 로드
...
새 터미널에서 프로젝트 루트로 이동한 뒤, 두 workload를 각각 실행한다.
