"""Chat과 Agent 원본을 로드하고 공통 request record로 정규화한다."""
...
def load_dataset(config: dict[str, Any]) -> list[dict[str, Any]]:
    """두 원본 데이터셋을 workload type이 붙은 하나의 list로 로드한다."""
...
def normalize_dataset(
    ...
    """raw records를 Chat/Agent 공통 request schema로 변환한다."""
    # workload 및 샘플링 설정
    ...
    # workload별 row 분류
    rows_by_workload = {workload: [] for workload in ("chat", "agent")}
