...
| Agent-only APC ON / OFF | `agent_only_exp2_cap20_apc_on_len12288_summary.json` / `..._off_...json` |
...
| Chat+Agent mixed APC ON / OFF | `chat_traj_agent_exp2_cap20_apc_on_len12288_summary.json` / `..._off_...json` |
...
| Chat+Agent mixed eviction log | `eviction_logs/Eviction_chat_traj_agent_exp2_cap20_apc_on_len12288.jsonl` |
...
- SLO: Chat/RAG `400ms`, Longctx `7700ms`, Agent `10000ms`
- Context: RAG/Longctx 조건은 file name 기준 `len8192`, Agent 조건은 `len12288`
...
| 확인 항목 | 관측 결과 | 판단 |
|---|---|---|
| APC gain | Chat p50 TTFT gain이 single `40.20ms`에서 RAG mixed `13.98ms`, Longctx mixed `5.98ms`로 감소. Agent mixed는 queue delay scale이 커서 gain도 크게 계산됨 | RAG/Longctx에서는 mixed에서 prefix cache 이득이 유지되지 않음 |
| Chat hit rate | APC ON 기준 single `0.280`에서 RAG `0.088`, Longctx `0.082`, Agent `0.072`로 감소 | mixed에서 Chat cache hit가 크게 감소 |
| Chat TTFT | APC ON 기준 p50 `113.35ms`에서 RAG `242.65ms`, Longctx `419.07ms`, Agent `4187.52ms`로 증가 | mixed에서 Chat TTFT 증가, Agent mixed가 가장 큼 |
| Chat SLO | APC ON 기준 `92.3%`에서 RAG `83.7%`, Longctx `47.1%`, Agent `27.2%`로 감소 | mixed에서 Chat SLO 감소, Agent mixed가 가장 큼 |
| Antagonist TTFT/SLO | RAG는 mixed에서 TTFT 증가와 SLO 감소, Longctx와 Agent는 현재 조건에서 mixed SLO가 높게 관측됨 | Chat 손해와 antagonist 손해가 대칭적이지 않음 |
| Useful eviction | `chat <- rag` useful eviction `14,690`, `chat <- longctx` `23,448`, `chat <- agent` `6,358`; Agent self useful eviction은 `53,191` | Agent도 Chat cache를 밀어내지만, 더 큰 신호는 Agent self-reuse |
...
*Figure 1. Chat의 조건별 APC gain. Chat+Agent는 절대 latency scale이 커서 broken axis로 표시한다.*

| Metric | Chat-only APC gain | Mixed Chat (RAG) | Mixed Chat (Longctx) | Mixed Chat (Agent) |
|---|---:|---:|---:|---:|
| TTFT mean | 22.75ms | 9.57ms | 17.43ms | 279.91ms |
| TTFT p50 | 40.20ms | 13.98ms | 5.98ms | 237.94ms |
| TTFT p95 | 14.89ms | 5.28ms | 11.98ms | 407.45ms |
| TTFT p99 | 53.70ms | 22.64ms | 95.73ms | 2544.30ms |
| SLO attainment | +1.8pp | -0.4pp | +0.7pp | +0.5pp |
...
> [!NOTE]
> Mixed Chat+Longctx의 TTFT p99 gain(`95.73ms`)은 single(`53.70ms`)보다 커 보인다. Chat+Agent의 gain도 절대 queue delay가 큰 run에서 계산되므로, cache 효과 판단은 아래 hit rate와 SLO를 함께 본다.
...
*Figure 2. Chat의 APC ON cache hit rate. single `0.280`에서 mixed 조건들은 `0.07-0.09` 수준으로 떨어진다.*
...
| Chat + Agent | 0.072 | -74.3% |

결론은 명확하다. mixed에서는 Chat hit rate가 single 대비 약 `70-74%` 감소한다.

세 mixed 조건의 차이는 hit rate보다 eviction 방향과 serving 지표에서 더 선명하다. RAG와 Longctx의 hit rate는 `0.088`과 `0.082`로 비슷하지만, Longctx는 뒤에서 보듯 Chat hot cache를 더 많이 직접 evict하고 Chat TTFT/SLO 손해도 크게 만든다. Agent mixed는 hit rate가 `0.072`까지 내려가며, Chat과 Agent 양쪽의 reusable cache가 서로 압박하는 패턴을 보인다.

## 6. 확인 2: mixed에서 Chat TTFT가 증가하는가?

APC ON 기준으로 Chat TTFT는 mixed에서 증가한다. 특히 Agent와 섞였을 때 증가 폭이 크다.

![Case 1 Chat APC ON TTFT](case1_analysis_image3_chat_ttft_apc_on.png)

*Figure 3. Chat의 APC ON TTFT p50/p95/p99를 조건별로 비교한다. Chat+Agent는 절대 latency scale이 커서 broken axis로 표시한다.*
...
| Chat + Agent | 5588.04ms | 4187.52ms | 16333.91ms | 22652.66ms |

Chat p50 TTFT는 single `113.35ms`에서 RAG mixed `242.65ms`, Longctx mixed `419.07ms`, Agent mixed `4187.52ms`로 증가한다. p95와 p99에서도 Agent mixed가 가장 나쁘다.
...
*Figure 4. Chat의 APC ON TTFT SLO attainment. single `92.3%`에서 RAG mixed `83.7%`, Longctx mixed `47.1%`, Agent mixed `27.2%`로 감소한다.*
...
| Chat + Agent | 27.2% | -65.1pp |

Chat SLO는 RAG mixed에서 `83.7%`로 낮아지고, Longctx mixed에서는 `47.1%`, Agent mixed에서는 `27.2%`까지 떨어진다. 즉 mixed에서 Chat의 user-facing 품질 손해가 관측되며, Agent mixed에서 그 손해가 가장 크다.
...
| Agent | Agent-only | 15107.95ms | 13061.72ms | 30631.71ms | 34.6% |
| Agent | Chat + Agent | 12003.33ms | 11531.20ms | 26287.45ms | 44.7% |

RAG는 mixed에서 TTFT가 증가하고 SLO가 `99.5% -> 88.3%`로 감소한다. 반면 Longctx는 현재 조건에서 mixed SLO가 `100.0%`로 유지된다. Agent도 single 대비 mixed에서 mean TTFT가 `15107.95ms -> 12003.33ms`로 낮고 SLO가 `34.6% -> 44.7%`로 높게 관측된다.
...
| Chat+Agent mixed APC ON | 126,793 |

Longctx mixed의 전체 eviction은 RAG mixed의 약 `2.01x`다. 이는 Longctx가 RAG보다 cache pressure가 큰 antagonist라는 실험 의도와 맞는다. Agent mixed의 전체 eviction은 `126,793`건으로 RAG보다 많고 Longctx보다 적다.
...
| Chat + Agent | 49,367 | 9,370 | 19.0% | 6,358 | 67.9% |
...
- `chat <- agent`: `9,370`건, Chat eviction 중 `19.0%`

그중 이후 다시 쓰인 `reused_later` 비율은 RAG와 Longctx에서 약 `80%`로 높고, Agent에서도 `67.9%`로 낮지 않다.

- `chat <- rag`: useful eviction `14,690`건, useful ratio `79.0%`
- `chat <- longctx`: useful eviction `23,448`건, useful ratio `79.9%` (`1.60x`)
- `chat <- agent`: useful eviction `6,358`건, useful ratio `67.9%`
...
Agent mixed는 다른 패턴을 보인다. `chat <- agent` useful eviction은 `6,358`건으로 RAG/Longctx보다 적지만, `agent <- agent` useful eviction은 `53,191/68,855 = 77.3%`다. 즉 Agent는 Chat cache를 일부 직접 밀어내는 동시에, Agent 자신의 multi-step session prefix도 나중에 다시 쓰이는 구조를 가진다.

5절의 Chat hit rate도 같은 방향을 보인다. RAG mixed `0.088`, Longctx mixed `0.082`, Agent mixed `0.072`로 모두 single 대비 크게 낮다. Longctx는 `chat <- antagonist` useful eviction을 가장 많이 만들며 Chat TTFT/SLO 손해도 크게 키운다. Agent는 Chat을 직접 밀어내는 양은 작지만, Agent session 내부에서도 useful eviction이 크게 발생해 양쪽 cache가 서로 압박하는 패턴을 만든다.

## 10. 1차 결론

Case 1의 핵심 체크리스트 기준으로는 다음 결론을 낼 수 있다.

1. Mixed workload에서는 Chat의 APC gain이 single workload만큼 유지되지 않는다. Chat p50 TTFT gain은 single `40.20ms`에서 RAG mixed `13.98ms`, Longctx mixed `5.98ms`로 감소한다. Agent mixed의 gain은 queue delay scale이 커서 hit rate와 SLO를 함께 해석한다.
2. Mixed workload에서는 Chat hit rate가 single 대비 약 `70-74%` 감소한다.
3. Mixed workload에서는 Chat TTFT가 증가하고 SLO attainment가 감소한다. Chat SLO는 Longctx mixed에서 `47.1%`, Agent mixed에서 `27.2%`까지 떨어진다.
4. Antagonist 자신의 손해는 Chat과 대칭적이지 않다. RAG는 mixed에서 SLO가 떨어지지만, Longctx와 Agent는 현재 조건에서 mixed SLO가 single보다 높게 관측된다.
...
6. Agent mixed에서는 `chat <- agent` useful eviction이 `6,358`건으로 관측된다. 동시에 `agent <- agent` useful eviction이 `53,191`건으로 커서, Agent workload는 Chat cache pressure와 Agent self-reuse를 동시에 만든다.

따라서 Case 1은 `HYPOTHESIS.md`의 핵심 주장, 즉 mixed workload에서 prefix cache 이득이 약해지고, RAG보다 큰 pressure를 주는 Longctx가 Chat hot cache useful eviction을 더 강하게 유발한다는 설명을 지지한다. Agent 확장 결과는 여기에 더해, multi-step Agent workload가 Chat 품질을 크게 악화시키면서도 자기 prefix reuse를 가진 별도 보호 대상임을 보여준다.
...
| single | agent | ON | 0.139 | 15107.95ms | 13061.72ms | 30631.71ms | 33320.55ms | 61.14ms | 34.6% |
| single | agent | OFF | - | 16404.32ms | 15213.89ms | 32090.41ms | 34191.78ms | 62.50ms | 28.2% |
...
| mixed (agent) | chat | ON | 0.072 | 5588.04ms | 4187.52ms | 16333.91ms | 22652.66ms | 71.50ms | 27.2% |
| mixed (agent) | chat | OFF | - | 5867.95ms | 4425.46ms | 16741.36ms | 25196.96ms | 74.19ms | 26.7% |
| mixed (agent) | agent | ON | 0.123 | 12003.33ms | 11531.20ms | 26287.45ms | 29827.58ms | 80.34ms | 44.7% |
| mixed (agent) | agent | OFF | - | 12954.77ms | 12157.74ms | 27134.50ms | 30507.58ms | 80.47ms | 42.7% |

</details>

### 11.2 Antagonist의 hit rate는 workload별로 다르게 유지된다

Chat과 달리, RAG/Longctx 자신은 mixed에서 cache hit rate가 거의 변하지 않는다. Agent는 mixed에서도 hit rate가 `0.123`으로 유지되지만, single `0.139` 대비 약간 낮아진다.

| Metric | RAG-only | Mixed RAG | Longctx-only | Mixed Longctx | Agent-only | Mixed Agent |
|---|---:|---:|---:|---:|---:|---:|
| hit_rate_mean | 0.101 | 0.101 | 0.029 | 0.029 | 0.139 | 0.123 |

![Case 1 Antagonist Cache Hit Rate](case1_analysis_image9_antagonist_cache_hit.png)

*Figure 9. antagonist(RAG/Longctx/Agent) 자신의 single vs mixed. RAG/Longctx hit rate는 거의 유지되고, Agent hit rate는 mixed에서도 상당 부분 유지된다.*
